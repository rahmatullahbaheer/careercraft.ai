...
const RegistrationFormNewContent = () => {
  // Add mounted state to prevent hydration issues - must be first hook
  ...
  // Stabilize the profile requirement to prevent hook order changes
  ...
  useEffect(() => {
    setIsProfileRequired(getProfile === "true");
  }, [getProfile]);

  // Create a stable validation schema that always includes all fields
  const validationSchema = useMemo(() => {
    return Yup.object({
      ...
      // Conditionally require profile fields
      expectedSalary: Yup.string().when([], {
        is: () => isProfileRequired,
        then: (schema) => schema.required("Expected salary is required"),
        otherwise: (schema) => schema.notRequired(),
      }),
      desiredJobTitle: Yup.string().when([], {
        is: () => isProfileRequired,
        then: (schema) => schema.required("Desired job title is required"),
        otherwise: (schema) => schema.notRequired(),
      }),
      preferedLocation: Yup.string().when([], {
        is: () => isProfileRequired,
        then: (schema) => schema.required("Preferred location is required"),
        otherwise: (schema) => schema.notRequired(),
      }),
  ...
  // Store formik reference to avoid dependency issues
  ...
  const fetchRegistrationDataFromResume = useCallback(
    async (content) => {
      setFileError("");
        ...
        fetch("/api/homepage/fetchRegistrationDataForHomepage", {
          ...
          .then(async (resp) => {
            ...
            if (res.success) {
              ...
              // Don't call formik.setFieldValue here - let the useEffect handle it
              // This prevents the dependency issue
            } else {
  ...
  // Use ref to track if params have been processed to prevent infinite loops
  ...
  useEffect(() => {
    ...
    // Create a stable key to compare params
    ...
    // Only update if params actually changed and formik is available
    if (lastParamsRef.current !== currentParamsKey && formikRef.current) {
      lastParamsRef.current = currentParamsKey;

      // Use setTimeout to avoid setting fields during render
      setTimeout(() => {
        const formikInstance = formikRef.current;
        if (firstName && formikInstance?.setFieldValue)
          formikInstance.setFieldValue(
            "firstName",
            removeDashesFromString(firstName)
          );
        if (lastName && formikInstance?.setFieldValue)
          formikInstance.setFieldValue(
            "lastName",
            removeDashesFromString(lastName)
          );
        if (email && formikInstance?.setFieldValue)
          formikInstance.setFieldValue("email", removeDashesFromString(email));
        if (registeredPhone && formikInstance?.setFieldValue)
          formikInstance.setFieldValue(
            "phone",
            removeDashesFromString(registeredPhone)
          );
        if (file && formikInstance?.setFieldValue)
          formikInstance.setFieldValue("file", file);
      }, 0);
    }
  }, [params.toString()]); // Use params.toString() for stable dependency
  ...
  // Show loading state during hydration to prevent hook order issues
  return (
...
const RegistrationFormNew = () => {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <RegistrationFormNewContent key="signup-form" />
    </Suspense>
  );
};
